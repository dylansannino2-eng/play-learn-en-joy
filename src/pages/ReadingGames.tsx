      ...
      <main className="ml-16 pt-20 px-6 pb-8 space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Mejora tu comprensión lectora y amplía tu vocabulario con estos desafíos.
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {games.map((game) => (
              <GameCard key={game.id} id={game.id} slug={game.slug ?? undefined} title={game.title} image={game.image} badge={game.badge} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-20 text-muted-foreground border-2 border-dashed rounded-2xl">
            ...
            <p>Aún no hay juegos de Reading disponibles. ¡Estamos preparando nuevas lecturas!</p>
          </div>
        )}
      </main>
    </div>
